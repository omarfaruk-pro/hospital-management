...
import { addNewDoctor, updateDoctor } from "@/app/actions/doctor";
...
export default function EditDoctorPage() {
        ...
        const fetchDoctor = async () => {
            ...
            if (res._id) {
                const getData = {
                    ...
                    feeNew: res.fee.newPatient,
                    feeOld: res.fee.oldPatientWithin2Months,
                    assistantName: res.emergencyContact.name,
                    assistantPhone: res.emergencyContact.phone,
                    status: res.status,
                    about: res.about,
                }
                setForm(getData);
                setSelectedDepartments(res.departments.map((d) => d._id));
                setEducations(res.education || [{ degree: "", institute: "", country: "", year: "" }]);
                setExperiences((!res.workExperiences || res.workExperiences.length ===0 )? [{ hospital: "", designation: "", from: "", to: "" }] : res.workExperiences);
                setSchedule(res.schedule || [{ day: "", startTime: "", endTime: "", maxPatients: "" }]);
            }
        }
        fetchDoctor();
    }, [doctorId]);
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await updateDoctor(doctorId, payload);
        if (res.success) {
            Swal.fire({ icon: "success", title: "UPDATED", text: "Doctor updated successfully" });
        } else {
                ...
                <form onSubmit={handleSubmit} className="space-y-10 p-10 bg-white rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <input type="text" value={form.name} name="name" placeholder="Full Name" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold" />
                            <input type="email" value={form.email} name="email" placeholder="Email Address" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold" />
                            <input type="tel" value={form.phone} name="phone" placeholder="Phone Number" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold" />

                            <input type="text" value={form.registrationNumber} name="registrationNumber" placeholder="BMDC Registration" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold" readOnly />
                            <input type="number" value={form.feeNew} name="feeNew" placeholder="New Patient Fee" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold" />
                            <input type="number" value={form.feeOld} name="feeOld" placeholder="Old Patient Fee" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold" />


                            <select name="status" value={form.status} onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold appearance-none text-gray-500">
                                <option value="">Select Status</option>
                                <option value="active">Active</option>
                                <option value="inactive">Inactive</option>
                            </select>

                            <input type="text" value={form.experienceYears} name="experienceYears" placeholder="Experience Years" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold" />
                            ...
                            <input type="text" name="gender" value={form.gender} onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold text-gray-500" readOnly />

                            <input type="text" value={form.designation} name="designation" placeholder="Designation" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold" />
                        ...
                        <div className="space-y-4">
                            ...
                            {schedule.map((_, i) => (
                                ...
                                <div key={i} className="p-6 bg-bg/50 rounded-3xl border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-4 relative group items-end">
                                    ...
                                    <select placeholder="Day" value={schedule[i].day} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                    ...
                                    <label htmlFor="startTime">
                                        Start Time
                                        <input id="startTime" value={schedule[i].startTime} placeholder="Start Time" type="time" className=" p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm block w-full" onChange={(e) => {
                                            const newSch = [...schedule];
                                    ...
                                    <label htmlFor="endTime">
                                        End Time
                                        <input id="endTime" value={schedule[i].endTime} placeholder="End Time" type="time" className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm block w-full" onChange={(e) => {
                                            const newSch = [...schedule];
                                    ...
                                    <input placeholder="Max Patients" value={schedule[i].maxPatients} type="number" className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newSch = [...schedule];
                        ...
                        <div className="space-y-4">
                            {educations.map((edu, i) => (
                                <div key={i} className="p-6 bg-bg/50 rounded-3xl border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-4 relative group">
                                    ...
                                    <input placeholder="Degree" value={educations[i].degree} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newEdu = [...educations];
                                        newEdu[i].degree = e.target.value;
                                        setEducations(newEdu);
                                    }} />

                                    <input placeholder="Institute" value={educations[i].institute} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newEdu = [...educations];
                                        newEdu[i].institute = e.target.value;
                                        setEducations(newEdu);
                                    }} />

                                    <input placeholder="Country" value={educations[i].country} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newEdu = [...educations];
                                        newEdu[i].country = e.target.value;
                                        setEducations(newEdu);
                                    }} />

                                    <input placeholder="Year" value={educations[i].year} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newEdu = [...educations];
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            {experiences.map((exp, i) => (
                                <div key={i} className="p-6 bg-bg/50 rounded-3xl border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-4 relative">
                                    ...
                                    <input placeholder="Hospital" value={experiences[i].hospital} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newExp = [...experiences];
                                        newExp[i].hospital = e.target.value;
                                        setExperiences(newExp);
                                    }} />

                                    <input placeholder="Designation" value={experiences[i].designation} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newExp = [...experiences];
                                        newExp[i].designation = e.target.value;
                                        setExperiences(newExp);
                                    }} />

                                    <input placeholder="From (Year)" value={experiences[i].from} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newExp = [...experiences];
                                        newExp[i].from = e.target.value;
                                        setExperiences(newExp);
                                    }} />

                                    <input placeholder="To (Year)" value={experiences[i].to} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" onChange={(e) => {
                                        const newExp = [...experiences];
                        ...
                        <div className="space-y-4">
                            <div className="p-6 bg-bg/50 rounded-3xl border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-4 relative">
                                <input type="text" value={form.assistantName} placeholder="Assistant Name" name="assistantName" onChange={handleChange} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" />
                                <input type="tel" value={form.assistantPhone} placeholder="Assistant Phone" name="assistantPhone" onChange={handleChange} className="p-3 bg-white border border-gray-100 rounded-xl font-bold text-sm" />
                            </div>
                        </div>
                    </div>

                    <div>
                        <textarea name="about" value={form.about} placeholder="Write about doctor shortly" rows="5" onChange={handleChange} className="w-full p-4 bg-bg rounded-2xl outline-none focus:ring-2 ring-primary/10 transition-all font-bold"></textarea>
                    </div>
